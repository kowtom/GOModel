	...
	}{
		{
			...
			checkFn: func(t *testing.T, req *core.ChatRequest) {
				if req.Model != "llama-3.3-70b-versatile" {
					t.Errorf("Model = %q, want %q", req.Model, "llama-3.3-70b-versatile")
				}
...
func TestConvertChatResponseToResponses(t *testing.T) {
	...
	if result.Model != "llama-3.3-70b-versatile" {
		t.Errorf("Model = %q, want %q", result.Model, "llama-3.3-70b-versatile")
	}
...
func TestNewWithHTTPClient(t *testing.T) {
	...
	provider := NewWithHTTPClient(apiKey, customClient)
