...
The dashboard's provider status (Healthy / Degraded / Unhealthy and the
"Last checked" timestamp) is **not** the circuit breaker. Circuit breaker
state is not shown in the dashboard and recovers on its own within
`timeout` (default 30s) once the provider is back; when
[metrics](/advanced/configuration#metrics) are enabled it is exported as
